...
import com.konstantinkochetov.beer.order.service.domain.BeerOrderEventEnum;
...
import com.konstantinkochetov.beer.order.service.sm.BeerOrderStateChangeInterceptor;
...
public class BeerOrderManagerImpl implements BeerOrderManager {
    ...
    public BeerOrder newBeerOrder(BeerOrder beerOrder) {
        beerOrder.setId(null); // cuz we don't need it when creating new
        beerOrder.setOrderStatus(BeerOrderStatusEnum.NEW);

        BeerOrder savedBeerOrder = beerOrderRepository.saveAndFlush(beerOrder); // hibernate will create UUID
    ...
    private StateMachine<BeerOrderStatusEnum, BeerOrderEventEnum> build(BeerOrder beerOrder){
        StateMachine<BeerOrderStatusEnum, BeerOrderEventEnum> sm = stateMachineFactory.getStateMachine(beerOrder.getId()); // spring will do caching if we already working with it (it will get us old one or build a new)

        sm.stop(); // we don't need what sm says we need what database says (refresh)
